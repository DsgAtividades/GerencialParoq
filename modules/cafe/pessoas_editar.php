    ...
    try {
        ...
        if (!empty($novo_cartao) && $novo_cartao !== $pessoa['qrcode']) {
            ...
            $stmt = $pdo->prepare("
                ...
                ");
            $stmt->execute([$_SESSION['usuario_nome'],$_SESSION['usuario_grupo'],$custo_cartao,$id_pessoa,$pessoa['qrcode'] . ' | '. $novo_cartao]);
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" class="needs-validation" novalidate>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-primary" id="updatePessoa">
...
function desabilita(){
    var button = document.getElementById('updatePessoa');
...
function habilita(){
    ...
    button.innerText = 'Salvar Alterações'; // Opcional
}
    ...
    .then(data => {
        if (data.success && data.participante && data.participante.cartao_codigo === decodedText) {
            ...
            if (data.participante.id == <?= $id_pessoa ?>) {
                ...
                alert('Cartão já está vinculado a este participante.');
                desabilita();

            } else {
                showQrStatus('Este cartão já está vinculado a outro participante!', 'danger');
                alert('Este cartão já está vinculado a outro participante!');
                desabilita();
            }
            ...
            habilita();
            //alert('Cartão disponível para troca.');
        } else {
