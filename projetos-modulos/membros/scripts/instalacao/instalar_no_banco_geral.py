#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Script para instalar as tabelas do módulo de Membros no banco principal gerencialparoq
Sistema de Gestão Paroquial
"""

import mysql.connector
import sys
import os
from datetime import datetime

class InstaladorMembros:
    def __init__(self):
        # Configurações do banco principal
        self.config = {
            'host': 'gerencialparoq.mysql.dbaas.com.br',
            'database': 'gerencialparoq',
            'user': 'gerencialparoq',
            'password': 'Dsg#1806',
            'charset': 'utf8mb4',
            'collation': 'utf8mb4_unicode_ci'
        }
        
        self.connection = None
        self.cursor = None
        
    def conectar(self):
        """Conecta ao banco de dados"""
        try:
            print("[INFO] Conectando ao banco gerencialparoq...")
            self.connection = mysql.connector.connect(**self.config)
            self.cursor = self.connection.cursor()
            print("[OK] Conexao estabelecida com sucesso!")
            return True
        except mysql.connector.Error as e:
            print(f"[ERRO] Erro ao conectar: {e}")
            return False
    
    def verificar_banco(self):
        """Verifica se o banco existe e está acessível"""
        try:
            self.cursor.execute("SELECT DATABASE()")
            banco_atual = self.cursor.fetchone()[0]
            print(f"📊 Banco atual: {banco_atual}")
            
            if banco_atual != 'gerencialparoq':
                print("⚠️  Aviso: Não está conectado ao banco gerencialparoq")
                return False
            
            # Verificar se já existem tabelas do módulo
            self.cursor.execute("""
                SELECT COUNT(*) 
                FROM information_schema.tables 
                WHERE table_schema = 'gerencialparoq' 
                AND table_name LIKE 'membros_%'
            """)
            tabelas_existentes = self.cursor.fetchone()[0]
            
            if tabelas_existentes > 0:
                print(f"⚠️  Encontradas {tabelas_existentes} tabelas do módulo já existentes")
                resposta = input("Deseja continuar mesmo assim? (s/N): ").lower()
                if resposta != 's':
                    print("❌ Instalação cancelada pelo usuário")
                    return False
            
            return True
        except mysql.connector.Error as e:
            print(f"❌ Erro ao verificar banco: {e}")
            return False
    
    def executar_sql_file(self, arquivo_sql):
        """Executa um arquivo SQL"""
        try:
            print(f"📄 Executando arquivo: {arquivo_sql}")
            
            with open(arquivo_sql, 'r', encoding='utf-8') as file:
                sql_content = file.read()
            
            # Dividir o conteúdo em statements individuais
            statements = [stmt.strip() for stmt in sql_content.split(';') if stmt.strip()]
            
            sucessos = 0
            erros = 0
            
            for i, statement in enumerate(statements, 1):
                if not statement or statement.startswith('--'):
                    continue
                
                try:
                    self.cursor.execute(statement)
                    self.connection.commit()
                    sucessos += 1
                    
                    # Mostrar progresso para statements importantes
                    if any(keyword in statement.upper() for keyword in ['CREATE TABLE', 'INSERT INTO', 'CREATE INDEX']):
                        print(f"  ✅ Statement {i} executado")
                        
                except mysql.connector.Error as e:
                    erros += 1
                    print(f"  ❌ Erro no statement {i}: {e}")
                    # Continuar com os próximos statements
                    continue
            
            print(f"📊 Resultado: {sucessos} sucessos, {erros} erros")
            return erros == 0
            
        except FileNotFoundError:
            print(f"❌ Arquivo não encontrado: {arquivo_sql}")
            return False
        except Exception as e:
            print(f"❌ Erro ao executar arquivo SQL: {e}")
            return False
    
    def verificar_instalacao(self):
        """Verifica se a instalação foi bem-sucedida"""
        try:
            print("\n🔍 Verificando instalação...")
            
            # Verificar tabelas criadas
            self.cursor.execute("""
                SELECT COUNT(*) 
                FROM information_schema.tables 
                WHERE table_schema = 'gerencialparoq' 
                AND table_name LIKE 'membros_%'
            """)
            total_tabelas = self.cursor.fetchone()[0]
            print(f"📊 Total de tabelas criadas: {total_tabelas}")
            
            # Verificar dados iniciais
            tabelas_dados = [
                ('membros_habilidades_tags', 'Habilidades'),
                ('membros_formacoes', 'Formações'),
                ('membros_funcoes', 'Funções'),
                ('membros_pastorais', 'Pastorais')
            ]
            
            for tabela, nome in tabelas_dados:
                try:
                    self.cursor.execute(f"SELECT COUNT(*) FROM {tabela}")
                    count = self.cursor.fetchone()[0]
                    print(f"  ✅ {nome}: {count} registros")
                except mysql.connector.Error:
                    print(f"  ❌ {nome}: Tabela não encontrada")
            
            # Verificar índices
            self.cursor.execute("""
                SELECT COUNT(*) 
                FROM information_schema.statistics 
                WHERE table_schema = 'gerencialparoq' 
                AND table_name LIKE 'membros_%'
                AND index_name != 'PRIMARY'
            """)
            total_indices = self.cursor.fetchone()[0]
            print(f"📊 Total de índices criados: {total_indices}")
            
            return total_tabelas >= 20  # Esperamos pelo menos 20 tabelas
            
        except mysql.connector.Error as e:
            print(f"❌ Erro ao verificar instalação: {e}")
            return False
    
    def criar_backup_antes(self):
        """Cria backup antes da instalação"""
        try:
            print("💾 Criando backup antes da instalação...")
            
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_file = f"backup_antes_instalacao_membros_{timestamp}.sql"
            
            # Comando mysqldump (se disponível)
            import subprocess
            cmd = [
                'mysqldump',
                f'--host={self.config["host"]}',
                f'--user={self.config["user"]}',
                f'--password={self.config["password"]}',
                '--single-transaction',
                '--routines',
                '--triggers',
                self.config['database']
            ]
            
            with open(backup_file, 'w', encoding='utf-8') as f:
                result = subprocess.run(cmd, stdout=f, stderr=subprocess.PIPE, text=True)
            
            if result.returncode == 0:
                print(f"✅ Backup criado: {backup_file}")
                return True
            else:
                print(f"⚠️  Aviso: Não foi possível criar backup automático: {result.stderr}")
                return False
                
        except Exception as e:
            print(f"⚠️  Aviso: Erro ao criar backup: {e}")
            return False
    
    def instalar(self):
        """Executa a instalação completa"""
        print("=" * 60)
        print("INSTALACAO DO MODULO DE MEMBROS")
        print("   Banco: gerencialparoq")
        print("   Data:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        print("=" * 60)
        
        # 1. Conectar
        if not self.conectar():
            return False
        
        # 2. Verificar banco
        if not self.verificar_banco():
            return False
        
        # 3. Criar backup (opcional)
        self.criar_backup_antes()
        
        # 4. Executar instalação
        arquivo_sql = "instalar_tabelas_geral.sql"
        if not os.path.exists(arquivo_sql):
            print(f"❌ Arquivo não encontrado: {arquivo_sql}")
            return False
        
        print(f"\n📦 Executando instalação...")
        if not self.executar_sql_file(arquivo_sql):
            print("❌ Falha na instalação")
            return False
        
        # 5. Verificar instalação
        if not self.verificar_instalacao():
            print("❌ Verificação da instalação falhou")
            return False
        
        print("\n" + "=" * 60)
        print("🎉 INSTALAÇÃO CONCLUÍDA COM SUCESSO!")
        print("=" * 60)
        print("✅ Todas as tabelas do módulo de Membros foram criadas")
        print("✅ Dados iniciais foram inseridos")
        print("✅ Índices de performance foram criados")
        print("✅ Triggers de auditoria foram configurados")
        print("\n🌐 O módulo está pronto para uso!")
        print("   Acesse: http://localhost/PROJETOS/GerencialParoq/projetos-modulos/membros/")
        
        return True
    
    def fechar_conexao(self):
        """Fecha a conexão com o banco"""
        if self.cursor:
            self.cursor.close()
        if self.connection:
            self.connection.close()

def main():
    """Função principal"""
    instalador = InstaladorMembros()
    
    try:
        sucesso = instalador.instalar()
        sys.exit(0 if sucesso else 1)
    except KeyboardInterrupt:
        print("\n❌ Instalação cancelada pelo usuário")
        sys.exit(1)
    except Exception as e:
        print(f"❌ Erro inesperado: {e}")
        sys.exit(1)
    finally:
        instalador.fechar_conexao()

if __name__ == "__main__":
    main()
